...
parser.add_argument('-configFile','--configfile', help='config parameters XML file ',required=True)
args = parser.parse_args()

configFile  = args.configfile

hostBD = 'mariadbc.localsi'
bd     = 'phenopsis'
login  = 'fabre'
pwd    = 'juliette'
...
if envoiBD == 1:
    ...
    mysqlFile= "/tmp/meteo.phenopsis"+site+".sql"
    if lastloc > nbRow:
        ...
        for ligne in lecteur:
            ...
            for var in data:
                donneesAInserer+=1
                sql = "INSERT INTO MesureMeteo (idChambre,date,idVariable,valeur) VALUES ('" + \
                      site + "','" + dateBD + "','" + var + "'," +str(data[var])+");\n"
                dataToWrite += sql
        ...
        #ecriture Log
        ecriture_fichierLog(fichierLog, message)
        #if fichierLog != None:
        #    #ecriture des logs
        #    try:
